...
//20200506 yanz #206155 #206165 添加接口
//20201225 yanz #59326 添加设置用户信息接口
//20210111 yanz #S2101110212 添加int64接口
//20210823 yanz #202108130850 开放格尔相关的接口设置
//20210903 yanz #202109151517 为格尔国密去掉Create2BizMsg中下载证书的逻辑，新增接口
//20210915 yanz #202109150867 支持设置字符串形式功能号
//20210915 yanz #202109151577 增加设置JSON字符串格式域值接口
//20211026 yanz #202108150010 支持获取格尔接口最新错误号
//20211215 yanz #202112070778 信安世纪国密开放接口
//20220107 yanz #202111241582 格尔国密支持废除指定证书
...
struct IBizMessage : public IKnown
{
	...
	//20200506 yanz #206155 添加域值接口
    //设置主题名所在域信息
    virtual void FUNCTION_CALL_MODE SetTopicInfo(const char* lpTopicName) = 0;
    //获取主题名所在域信息
    virtual const char* FUNCTION_CALL_MODE GetTopicInfo() = 0;
    //设置过滤信息
    virtual void FUNCTION_CALL_MODE SetFilterInfo(const char* lpFilterInfo,int index=0) = 0;
    //获取过滤信息
    virtual const char* FUNCTION_CALL_MODE GetFilterInfo(int index=0) = 0;
	//20201225 yanz #59326 添加设置用户信息接口
	virtual void FUNCTION_CALL_MODE SetShardingInfo(const char* lpShardingInfo) = 0;
	//获取用户信息
	virtual const char* FUNCTION_CALL_MODE GetShardingInfo() = 0;
	//20210915 yanz #202109150867 支持设置字符串形式功能号
	//设置字符串形式功能号
	virtual void FUNCTION_CALL_MODE SetFunctionEx(const char* szFunction) = 0;
	//获取字符串形式功能号
	virtual const char * FUNCTION_CALL_MODE GetFunctionEx() = 0;
	//20210915 yanz #202109151577 增加设置JSON字符串格式域值接口
	//设置调试类型
	virtual void FUNCTION_CALL_MODE SetDebugType(const char* szString, int index) = 0;
	//获取调试类型
	virtual const char *FUNCTION_CALL_MODE GetDebugType(int index) = 0;
};
...
public:
	...
	*/
	...
	//20201021 yanz #6150 增加安全标志获取接口
	/**
	* 取安全状态
	* @return 返回安全状态
	*/
	virtual bool FUNCTION_CALL_MODE IsSafe() = 0;

	//20210823 yanz #202108130850 开放格尔相关的接口设置
	/**
	* 查询本地所有用户的所有证书
	* @param const char* cert_dir 证书存储路径
	* @param const char* cert_dfk 证书设备指纹
	* @param char* cert_buf 证书存储缓冲区（返回数据为Json格式）
	* @param int* cert_buf_len 缓冲区长度
	* @return 0表示成功，否则失败
	*/
	virtual int FUNCTION_CALL_MODE GEER_CertQueryAllLocal(const char* cert_dir, const char* cert_dfk, char* cert_buf, int* cert_buf_len) = 0;
	/**
	* 申请证书延期
	* @param const char* sks_user 用户账号(tid，终端/设备的唯一标识)
	* @param const char* sks_server 格尔网关地址(cds服务端url)
	* @param const char* sks_password 用户密码
	* @param const char* sks_pin 用户Pin码
	* @return 0表示成功，否则失败
	*/
	virtual int FUNCTION_CALL_MODE GEER_CertDelay(const char* sks_user, const char* sks_server, const char* sks_password, const char* sks_pin = NULL) = 0;
	/**
	* 申请销毁本地证书并通知CA吊销
	* @param const char* sks_user 用户账号(tid，终端/设备的唯一标识)
	* @param const char* sks_server 格尔网关地址(cds服务端url)
	...
	* @return 0表示成功，否则失败
	*/
	virtual int FUNCTION_CALL_MODE GEER_CertRevoke(const char* sks_user, const char* sks_server, const char* sks_password, const char* sks_pin = NULL) = 0;
	/**
	* 申请重置PIN码（重置即解锁）
	* @param const char* sks_user 用户账号(tid，终端/设备的唯一标识)
	* @param const char* sks_server 格尔网关地址(cds服务端url)
	* @param const char* sks_password 用户密码
	* @param const char* newPin 新的用户PIN码
	* @return 0表示成功，否则失败
	*/
	virtual int FUNCTION_CALL_MODE GEER_PinReset(const char* sks_user, const char* sks_server, const char* sks_password, const char* newPin) = 0;
	/**
	* 修改用户PIN码
	* @param const char* sks_user 用户账号(tid，终端/设备的唯一标识)
	* @param const char* sks_server 格尔网关地址(cds服务端url)
	* @param const char* oldPin 旧的用户PIN码
	* @param const char* newPin 新的用户PIN码
	* @param int* retryCount 剩余尝试次数
	* @return 0表示成功，否则失败
	*/
	virtual int FUNCTION_CALL_MODE GEER_ChangePin(const char* sks_user, const char* sks_server, const char* oldPin, const char* newPin, int* retryCount) = 0;
	/**
	* 验证设备PIN码
	* @param const char* sks_user 用户账号(tid，终端/设备的唯一标识)
	* @param const char* sks_server 格尔网关地址(cds服务端url)
	* @param const char* pin 用户PIN码
	* @param int* retryCount 返回剩余尝试次数
	* @return 0表示成功，否则失败
	*/
	virtual int FUNCTION_CALL_MODE GEER_VerifyPin(const char* sks_user, const char* sks_server, const char* pin, int* retryCount) = 0;
    ...
    * 如果应用不需要任何回调方法，则可向该方法传递NULL，而不必自定义回调类和对象
	*/
	virtual int FUNCTION_CALL_MODE Create2BizMsgGM(CCallbackInterface *lpCallback) = 0;
	///////////////////////////////////增加更多格尔接口///////////////////////////////////////
	/**
	* 查询用户在所有终端上的证书
	* @param const char* sks_user 用户账号(tid，终端/设备的唯一标识)
	...
	* @return 0表示成功，否则失败
	*/
	virtual int FUNCTION_CALL_MODE GEER_CertQueryAllEndpoint(const char* sks_user, const char* sks_server, const char* sks_password, char* cert_buf, int* cert_buf_len) = 0;
	/**
	* 在线获取证书的状态
	* @param const char* sks_user 用户账号(tid，终端/设备的唯一标识)
	* @param const char* sks_server 格尔网关地址(cds服务端url)
	* @param int localOnly 是否只获取本地证书状态，0为否，1为是
	* @param int* state 证书状态
	* @return 0表示成功，否则失败
	*/
	virtual int FUNCTION_CALL_MODE GEER_CertState(const char* sks_user, const char* sks_server, int localOnly, int* state) = 0;
	/**
	* 申请颁发证书（在线版）
	* @param const char* sks_user 用户账号(tid，终端/设备的唯一标识)
	* @param const char* sks_server 格尔网关地址(cds服务端url)
	...
	* @return 0表示成功，否则失败
	*/
	virtual int FUNCTION_CALL_MODE GEER_CertEnroll(const char* sks_user, const char* sks_server, const char* sks_password, const char* sks_pin) = 0;
	//20211026 yanz #202108150010 支持获取格尔接口最新错误号
	/**
	* 根据错误码获取错误信息及详细描述
	* @param int err_code 错误码
	* @param int charset 输出字符集, 0: gb2312, 1: utf8
	* @param char* err_msg 输出的错误信息
	* @param int* msg_len 错误信息缓冲区长度
	* @return 0表示成功，否则失败
	*/
	virtual int FUNCTION_CALL_MODE GEER_ErrString(int err_code, int charset, char* err_msg, int* msg_len) = 0;
	/**
	* 获取格尔接口最新的错误码
	* @return 返回错误码
	*/
	virtual int FUNCTION_CALL_MODE GetGEERLastErrorNo() = 0;
	// 20211102
	/**
	* 获取接入控制令牌
	* @param szService 服务名
	* @param szVerifyLib 校验库
	* @param iTimeOut 获取时间
	* @param szResult 传入的存放结果的buffer
	* @param iRstLen  传入的buffer的长度，并返回实际长度
	* @param bForce   强制获取
	* @return 返回0成功，结果放在szResult和iRstLen中，否则失败，详细错误信息放在szResult和iRstLen中
	*/
	virtual int FUNCTION_CALL_MODE GetCtst(const char* szService, const char* szVerifyLib, int iTimeOut, char * szResult, int *iRstLen, bool bForce = false) = 0;
	//20211215 yanz #202112070778 信安世纪国密开放接口
	/**
	* 获取信安世纪接口最新的错误码
	* @return 返回错误码
	*/
	virtual int FUNCTION_CALL_MODE GetXASJLastErrorNo() = 0;
	/**
	* 获取信安世纪所有证书列表
	* @param const char* if_user      SDK用户名
	* @param const char* if_serverIP      MAuth地址
	* @param const char* if_password      SDK用户密码
	* @param const char* if_serviceID      MAuth服务标识
	* @param const char* if_appID      MAuth AppID
	* @param const char* if_secretKey      MAuth SecretKey
	* @param char* cert_buf      证书存储缓冲区（返回数据为Json格式）
	* JSON字符串中，键为"certList"标识有效证书列表，每一个元素为一张证书的信息
	* 每个元素存在以下键，其值均为字符串：
	*  "username" : 用户名
	*  "imei"     : 设备标识
	*  "dn"       : 证书主题
	*  "sn"       : 证书标识，若需要调用XASJ_RevokeSignCert作废证书，传入该字段的值
	* @param int* cert_buf_len      缓冲区长度
	* @return int 0表示成功，否则失败
	*/
	virtual int FUNCTION_CALL_MODE XASJ_ListSignCert(const char* if_user, const char* if_serverIP, const char* if_password, const char* if_serviceID, const char* if_appID, const char* if_secretKey, char* cert_buf, int* cert_buf_len) = 0;
	/**
	 * 信安世纪废弃证书
	 * @param const char* if_user      SDK用户名
	 * @param const char* if_serverIP      MAuth地址
	 * @param const char* if_password      SDK用户密码
	 * @param const char* if_pin      SDK用户名口令
	 * @param const char* if_serviceID      MAuth服务标识
	 * @param const char* if_appID      MAuth AppID
	 * @param const char* if_secretKey      MAuth SecretKey
	 * @param const char* pCertID    证书标识，可从XASJ_ListSignCert中获取。不传值默认废弃本地证书
	 * @return int 0表示成功，否则失败 
	 */
	virtual int FUNCTION_CALL_MODE XASJ_RevokeSignCert(const char* if_user, const char* if_serverIP, const char* if_password, const char* if_pin, const char* if_serviceID, const char* if_appID, const char* if_secretKey, const char *pCertID = NULL) = 0;
	/**
	 * 信安世纪重置PIN
	 * @param const char* if_user      SDK用户名
	 * @param const char* if_serverIP      MAuth地址
	 ...
	 * @param const char* if_appID      MAuth AppID
	 * @param const char* if_secretKey      MAuth SecretKey
	 * @param const char* newPin      重新设置的SDK用户名口令
	 * @return int 0表示成功，否则失败 
	 */
	virtual int FUNCTION_CALL_MODE XASJ_PinReset(const char* if_user, const char* if_serverIP, const char* if_password, const char* if_serviceID, const char* if_appID, const char* if_secretKey, const char* newPin) = 0;
	/**
	 * 信安世纪修改PIN
	 * @param const char* if_user      SDK用户名
	 * @param const char* if_serverIP      MAuth地址
	 ...
	 * @param const char* if_appID      MAuth AppID
	 * @param const char* if_secretKey      MAuth SecretKey
	 * @param const char* oldPin      旧的SDK用户密码
	 * @param const char* newPin      重新设置的SDK用户名口令
	 * @return int 0表示成功，否则失败 
	 */
	virtual int FUNCTION_CALL_MODE XASJ_ChangePin(const char* if_user, const char* if_serverIP, const char* if_serviceID, const char* if_appID, const char* if_secretKey, const char* oldPin, const char* newPin) = 0;
	/**
	 * 信安世纪校验PIN
	 * @param const char* if_user      SDK用户名
	 * @param const char* if_serverIP      MAuth地址
	 ...
	 * @param const char* if_appID      MAuth AppID
	 * @param const char* if_secretKey      MAuth SecretKey
	 * @param int* retryCount    口令剩余可尝试次数
	 * @return int 0表示成功，否则失败 
	 */
	virtual int FUNCTION_CALL_MODE XASJ_VerifyPin(const char* if_user, const char* if_serverIP, const char* if_password, const char* if_pin, const char* if_serviceID, const char* if_appID, const char* if_secretKey, int* retryCount) = 0;
	/**
	 * 信安世纪检查证书状态
	 * @param const char* if_user      SDK用户名
	 * @param const char* if_serverIP      MAuth地址
	 ...
	 * @param const char* if_appID      MAuth AppID
	 * @param const char* if_secretKey      MAuth SecretKey
	 * @return int 0表示成功，否则失败，返回错误状态 
	 */
	virtual int FUNCTION_CALL_MODE XASJ_CheckCertStatus(const char* if_user, const char* if_serverIP, const char* if_password, const char* if_pin, const char* if_serviceID, const char* if_appID, const char* if_secretKey) = 0;
	/**
	 * 信安世纪下载证书
	 * @param const char* if_user      SDK用户名
	 * @param const char* if_serverIP      MAuth地址
	 ...
	 * @return int 0表示成功，否则失败
	 */
	virtual int FUNCTION_CALL_MODE XASJ_DownSignCert(const char* if_user, const char* if_serverIP, const char* if_password, const char* if_pin, const char* if_serviceID, const char* if_appID, const char* if_secretKey) = 0;
	/**
	 * 信安世纪更新证书
	 * @param const char* if_user      SDK用户名
	 * @param const char* if_serverIP      MAuth地址
	 ...
	 * @return int 0表示成功，否则失败
	 */
	virtual int FUNCTION_CALL_MODE XASJ_UpdateSignCert(const char* if_user, const char* if_serverIP, const char* if_password, const char* if_pin, const char* if_serviceID, const char* if_appID, const char* if_secretKey) = 0;
	/**
	* 信安世纪根据错误码获取错误信息
	* @param int err_code 错误码
	* @return 错误信息
	*/
	virtual const char * FUNCTION_CALL_MODE XASJ_StrError(int err_code) = 0;
	/**
	* 获取信安世纪库版本
	* @return 版本信息
	*/
	virtual const char * FUNCTION_CALL_MODE XASJ_Version() = 0;
	//20220107 yanz #202111241582 格尔国密支持废除指定证书
	/**
	* 格尔废除指定证书
	* @param const char* sks_user 用户账号(tid，终端/设备的唯一标识)
	* @param const char* sks_server 格尔网关地址(cds服务端url)
	* @param const char* sks_password 用户密码
	* @param const char* cert_cid 证书id
	* @return 0表示成功，否则失败
	*/
	virtual int FUNCTION_CALL_MODE GEER_CertRevokeByCid(const char* sks_user, const char* sks_server, const char* sks_password,const char* cert_cid) = 0;
};
...
/**
* 传入参数：address port
* 传出参数：lpResult，其中成功返回字段：conn_count net_delay net_speed 失败返回字段：error_no error_info
* 返回值：0 成功，非0失败
*/
int FUNCTION_CALL_MODE GetNetWorkSpeed(char * lpAddress, int port, IF2Packer * lpResult);

/***
 * 支持设置公共配置文件szCommCfgFile是文件名称，要带文件路径
 * 目前该配置文件只支持设置t2sdk绑定的udp端口
 * 注意:这个接口必须在NewConnection和t2sdk_library_init函数之前调用
 */
void FUNCTION_CALL_MODE SetCommCfgFile(const char *szCommCfgFile);
}
